...
chim = cv2.imread('images/fish.png')

chim_resized = cv2.resize(chim, (50, 50))
...
chim_gray = cv2.cvtColor(chim_resized,cv2.COLOR_BGR2GRAY)
ret, mask = cv2.threshold(chim_gray, 254, 255, cv2.THRESH_BINARY)
mask_inv = cv2.bitwise_not(mask)
...
cv2.imwrite('level/level_1.png', anhmau)
cv2.imshow('level_1', anhmau)
cv2.waitKey(0)
